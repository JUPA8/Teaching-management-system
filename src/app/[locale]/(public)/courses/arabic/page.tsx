...
export default function ArabicPage() {
  ...
  useEffect(() => {
    fetch('/api/courses?type=ARABIC_LANGUAGE&isActive=true&limit=20')
      .then((r) => r.json())
              ...
              <div className="space-y-4">
                {[
                  ...
                  >
                    ...
                    <div>
                      <h4 className="font-bold text-charcoal mb-1">{t(`arabic.teachingFeatures.${item.key}.title` as any)}</h4>
                      <p className="text-charcoal-light text-sm">{t(`arabic.teachingFeatures.${item.key}.description` as any)}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
